...
print(f"WORKING ON DIRECTORY {dir_name}")
success_count = 0

log_file_e = LOG_DIR / f"{dir_name}.log"  
yaml_files = [f.name for f in os.scandir(WORK_DIR) 
...
if len(yaml_files) != 0:
    ...
    for file in yaml_files:
        ...
        log_file = LOG_DIR / f"{dir_name}_{Path(file).stem}.log"
    ...
    print(f"  No YAML files in {dir_name}")
    with open(log_file_e, 'w') as log:
        log.write(f"No YAML files found in {dir_name}\n")


# Open the log file for writing
with open(log_file_e, 'w') as log:
    # Write all details to the log file
    log.write(f"Processed: {dir_name}\n")

    # Save ALL details to log
    log.write(f"YAML file success: {success_count}\n")
    log.write("="*60 + "\n")
    log.write(f"YAML file failures: {len(yaml_files) - success_count} ""\n")
    log.write("="*60 + "\n")
